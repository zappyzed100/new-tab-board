...
SINGLE_TEST_COMMAND = ["npx", "vitest", "run", "{file}"]   # 単一スロット（ts-react-crx@1）
...
LAYER_FORBIDDEN_IMPORTS += [
    ("src/lib/", re.compile(r"""from\s+['"][^'"]*newtab[^'"]*['"]"""),
     "src/lib は src/newtab を import してはいけない（依存は newtab → lib の一方向のみ — AGENTS.md §5）"),
]
...
DEPRECATED_PATTERNS[".ts"] = _TS_DEPRECATED
DEPRECATED_PATTERNS[".tsx"] = _TS_DEPRECATED
