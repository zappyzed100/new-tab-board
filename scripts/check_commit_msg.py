...
#   v2.25（G10）: 検査6・8と同じく TEST_PATH_PATTERNS 未充填なら不発（列充填で有効化）。
#   従来はこのバイパスが検査2だけ欠けており、TEST_PATH_PATTERNS 未充填のリポジトリ
#   （列充填前・またはキット原本自身）では `fix:` コミットが原理上常に拒否される
#   非対称なバグだった（他の soft 検査6・8との整合性の欠如）。
