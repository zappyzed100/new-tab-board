...
- **検査5（feat-without-plan — v2.6 soft 導入・v2.8 hard 昇格＝G14「意図の保存」）**:
  ...
  **列充填。空なら不発**＝`layer-violation` と同じ「列充填で有効化」）に **HEAD に無い
  ...
  `docs/plans/`。本プロジェクトは`PLAN.md`へリネーム済みのためバインディングで
  `PLAN\.md$`を追加充填している。置き場の規約はルート `AGENTS.md` §4）の差分がステージに無ければ、
...
対象外——「実測できなかった理由」を明示した上での意図的な保留）。旧版（v2.25 相当）
の docs/guardrails/GUARDRAILS.md から、キット更新（Phase 44〜50・v2.42〜v2.48）に伴う
正本の配置先移動（docs/guardrails/ → .guardrails/）で本節へ移植した記録——番号の
Phase 32 は移植元の kit-source Phase 32（v2.22 同梱）と衝突するが、旧版でも同様の
リポジトリ固有追記として意図的に別名前空間で使われていたものをそのまま踏襲する。
