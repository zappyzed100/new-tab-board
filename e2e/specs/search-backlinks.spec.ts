...
// ノートタブは撤去済み。全件がボード(実測masonry)で常時表示されるため、ノート名は
// ペイン先頭の見出し(.note-pane-title-input)で編集し、対象は列の先頭ペインで名指しする
// (列は<div>でなくセルの data-column-index で表す——layout.css参照。linear先頭2件=col0/col1の先頭)。
...
test("検索・バックリンクが連動して動く", async ({ context, newTabUrl }) => {
  ...
  const colFirst = (c: number) =>
    page.locator(`.note-cell[data-column-index="${c}"] [data-testid^="note-editor-area-"]`).first();
