...
test("複数タブで順にノートを作成・削除しても本文とノート集合が収束する", async ({
  context,
  newTabUrl,
}) => {
  test.slow();
  ...
  for (let index = 0; index < 3; index += 1) {
    ...
    await tab.goto(newTabUrl);
    await expect(tab.getByTestId("app-root")).toBeVisible();
    tabs.push(tab);
  }

  // 空状態から同時起動した各タブは一時的に別IDのA/B/Cを生成しうる。操作開始前に、
  // storage通知を経て全タブが同じID集合へ収束したことを確認する。
  let lastIdSignature = "";
  let stableIdRounds = 0;
  await expect
    .poll(async () => {
      const ids = await Promise.all(
        tabs.map((tab) =>
          tab
            ...
            .evaluateAll((panes) => panes.map((pane) => pane.getAttribute("data-testid")).sort()),
        ),
      );
      const signature = JSON.stringify(ids);
      const converged = ids.every((current) => JSON.stringify(current) === JSON.stringify(ids[0]));
      stableIdRounds = converged && signature === lastIdSignature ? stableIdRounds + 1 : 0;
      lastIdSignature = signature;
      return stableIdRounds >= 3;
    })
    .toBe(true);

  const inputPaneIds = await tabs[0]
    .locator('[data-testid^="note-editor-area-"][data-empty="true"]')
    .evaluateAll((panes) => panes.slice(0, 3).map((pane) => pane.getAttribute("data-testid")));
  expect(inputPaneIds).toHaveLength(3);

  const created: Array<{ paneTestId: string; body: string }> = [];
  for (const [index, tab] of tabs.entries()) {
    const paneTestId = inputPaneIds[index];
    if (!paneTestId) throw new Error("入力対象ノートのtestidを取得できません");
    const stablePane = tab.getByTestId(paneTestId);
    const body = `複数タブ作成本文${index + 1}`;
    await stablePane.locator(".cm-content").click();
    await tab.keyboard.type(body);
    await expect(stablePane.locator(".cm-content")).toContainText(body);
    created.push({ paneTestId, body });
    await Promise.all(
      tabs.map((otherTab) =>
        expect(otherTab.getByTestId(paneTestId).locator(".cm-content")).toContainText(body),
      ),
    );
  }

  // NO-LOG: E2Eが共有chrome.storageの収束を読み取り確認するだけで、製品I/O経路ではない。
  await expect
    .poll(() =>
      tabs[0].evaluate(async (expected) => {
        const stored = await chrome.storage.local.get("localData");
        const localData = stored.localData as
          { notes?: Array<{ id: string; content: string }> } | undefined;
        const notes = localData?.notes ?? [];
        return expected.every(({ paneTestId, body }) =>
          notes.some(
            (note) =>
              note.id === paneTestId.replace("note-editor-area-", "") && note.content === body,
          ),
        );
      }, created),
    )
  ...
  const removedId = removed.paneTestId.replace("note-editor-area-", "");
  await tabs[0].getByTestId(`delete-note-${removedId}`).click();

  // NO-LOG: E2Eが削除tombstoneと共有ノート集合の収束を読み取り確認するだけ。
  await expect
    .poll(() =>
      tabs[0].evaluate(async (noteId) => {
        const stored = await chrome.storage.local.get("localData");
        const localData = stored.localData as
          { notes?: Array<{ id: string }>; noteTombstones?: Record<string, number> } | undefined;
        return (
          typeof localData?.noteTombstones?.[noteId] === "number" &&
          !(localData?.notes ?? []).some((note) => note.id === noteId)
        );
      }, removedId),
    )
    .toBe(true);
  ...
  await Promise.all(
    tabs.flatMap((tab) => [
      expect(tab.getByTestId(removed.paneTestId)).toHaveCount(0),
      expect(tab.getByText("(競合コピー)", { exact: false })).toHaveCount(0),
      ...created
        .filter((candidate) => candidate !== removed)
        .map((item) =>
          expect(tab.getByTestId(item.paneTestId).locator(".cm-content")).toContainText(item.body),
        ),
    ]),
  );
});
...
test("要約ボタンはOpenRouter APIキー未設定なら案内を出し、勝手に実行しない", async ({
  context,
  newTabUrl,
}) => {
  const page = await context.newPage();
...
test("タグをふるボタンはOpenRouter APIキー未設定なら案内を出す", async ({ context, newTabUrl }) => {
  const page = await context.newPage();
  await page.goto(newTabUrl);
  await expect(page.getByTestId("app-root")).toBeVisible();
