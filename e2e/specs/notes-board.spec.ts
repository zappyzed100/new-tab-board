...
test("ノートペイン先頭でノート名を編集するとタブにも反映される", async ({ context, newTabUrl }) => {
  ...
  // レイアウト実測(CLAUDE.md): ①ノート名はペインの一番左上=操作ボタン行より上にある
  // ②枠(border)が見えない ③文字は既定より大きい(ユーザー指示)。
  const titleBox = await title.boundingBox();
  const pinBox = await firstPane.locator('[data-testid^="pin-note-"]').boundingBox();
  if (!titleBox || !pinBox) throw new Error("title/pin not visible");
  // ①名前はボタン行(ピン)より上の行にある(下端がピンの上端以下)。
  expect(titleBox.y + titleBox.height).toBeLessThanOrEqual(pinBox.y + 1);
  // ①名前の左端はボタン行の左端とほぼ揃う(左上に置かれている)。
  expect(titleBox.x).toBeLessThanOrEqual(pinBox.x + 4);
  // ②枠が見えない(borderの太さが0)。
  ...
  // ③文字は少し大きい(1.15rem ≈ 18px。既定14px程度より大きい)。
