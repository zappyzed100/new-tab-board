...
test("ノートペイン先頭でノート名を編集するとタブにも反映される", async ({ context, newTabUrl }) => {
  ...
  // レイアウト実測(CLAUDE.md): ノート名はMarkdownプレビューの左(同じ行なら左・折り返すなら上)に
  // あって重ならず、文字は既定より少し大きい(ユーザー指示「プレビューの左に・ちょっと大きな文字で」)。
  const titleBox = await firstPane.locator(".note-pane-title").boundingBox();
  const previewBox = await firstPane.locator('[data-testid^="toggle-preview-"]').boundingBox();
  if (!titleBox || !previewBox) throw new Error("title/preview not visible");
  const titlePrecedesPreview =
    titleBox.y + titleBox.height <= previewBox.y + 1 || // プレビューより上の行
    titleBox.x + titleBox.width <= previewBox.x + 1; // 同じ行でプレビューの左
  expect(titlePrecedesPreview).toBe(true);
  const titleFontPx = await firstPane
    .locator(".note-pane-title input")
    .evaluate((el) => parseFloat(getComputedStyle(el).fontSize));
  expect(titleFontPx).toBeGreaterThan(15); // 1.05rem ≈ 16.8px(既定14px程度より大きい)
});
...
test("🧹初期化でノートの内容が空に戻る(削除とは違いノートは残る)", async ({
  ...
  const tabCount = await page.locator('[data-testid^="note-tab-select-"]').count();
  ...
  await expect(page.locator('[data-testid^="note-tab-select-"]')).toHaveCount(tabCount);
});
