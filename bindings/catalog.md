...
>
> **テストファイル判別のキット自身デフォルト（v2.25・G10）**: `.claude/hooks/*.py`・
> `scripts/*.py` は採用列に関係なく常にキット自身のPythonインフラであり
> （`HEADER_REQUIRED_EXTS`が`.py`/`.sh`を常時対象とするのと同じ扱い）、その回帰テスト
> （`tests/test_*.py`・`tests/*_test.py`）は`scripts/repo_scan.py`の`TEST_PATH_PATTERNS`
> 初期値に常時含まれる。各列の paste-block はこれに**追記**するだけでよく、
> 列側で重複定義・除外する必要はない。
