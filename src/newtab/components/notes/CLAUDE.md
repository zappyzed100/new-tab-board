...
## NoteTabs は radix-ui の生Tabsを直接使う(Themesの子複製を避けるため)

`NoteTabs.tsx` だけは `@radix-ui/themes` ではなく下層の `radix-ui` の `Tabs` を asChild で
使う。Themesの `Tabs.Trigger` は子を可視+隠し計測用の2箇所に複製し、data-testidが二重化して
E2Eのクリックが不安定になる実害があったため(ファイル冒頭コメントに詳細)。追加/リネーム/削除/
タブD&Dはここ、ピン/一つ上へ/ドラッグ交換/ノート名編集/削除/対応済みチェックは各ペイン
(NoteEditorPane)側にもある(ユーザー指示: 書く側の先頭でノート名を編集・削除・済みにできる)。
ノート名は**タブとペイン先頭の2箇所どちらでも編集でき**、どちらも `updateNote({title})` に集約。
