...
列ごとの入れ物へ振り分けていた頃は、ノートが1件増減しただけでセルが別の列=**別の親DOM**へ移り、
Reactが再マウントしてCodeMirrorが破棄され、**入力中のカーソルが飛んで以降の打鍵が丸ごと消えた**
(ユーザー報告「ノートA辺りを操作すると配置が勝手に変わり操作中のノートがどこかに移る」)。
親が変わらなければ配置が変わってもCM6は生き残る。E2Eで列を名指しするときは
`.note-cell[data-column-index="N"]`(`note-column-N`のtestidは廃止)。
...
- 数えるのは**盤面全体のプレースホルダ数**で、「末尾に連続する空の数」ではない。末尾の連続数で
  数えると、空3つの**2番目以降へ入力した瞬間**に「末尾の空が足りない」と誤判定して補充が走り、
  前に取り残された空が余剰として**入力中ノートより前から削除**され、盤面が繰り上がって入力中の
  ペインが別の列へ飛ぶ(=CM6再マウントでカーソルと打鍵が消える。2026-07-23のユーザー報告)。
...
- 超過分は **order の高い方=末尾側から間引く**。前から間引くと後続が繰り上がって表示位置が
  動くため、削るのは必ず末尾側(2026-07-23。以前は低い方から削っていた)。
- 取り残された空は**消さない**。2番目の空へ入力すると前の空はその場に残り、末尾へ1つ補充されて
  合計3つになる(見た目は末尾に2つ+途中に1つ)。後続の入力で自然に末尾へ揃う。
