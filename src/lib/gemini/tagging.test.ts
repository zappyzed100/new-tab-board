// tagging.test.ts — OpenRouterによる自動タグ付けの単体テスト。実APIは叩かずfetchをフェイクにする。
...
describe("tagNote", () => {
  it("OpenRouterの応答からタグ配列を返す", async () => {
    const fetch = openRouterReply("旅行, 京都, 計画");
  ...
  it("タグ候補を渡すとプロンプトへ「優先的に選ぶ候補」として差し込まれる", async () => {
    const fetch = openRouterReply("コーディング");
  ...
  it("タグ候補が空ならプロンプトに候補の一節は入らない(従来どおり)", async () => {
    const fetch = openRouterReply("旅行");
...
describe("analyzeNote", () => {
  it("TAGS/TITLE/JUDGE形式からタグ・タイトル・ゴミ判定を取り出す", async () => {
    const fetch = openRouterReply("TAGS: 買い物, 牛乳\nTITLE: 買い物リスト\nJUDGE: OK");
