...
## notes/<id>.md は「タグ検索の正本」(YAML front matter + 本文)

`writeNoteMarkdownToNas`/`noteToMarkdown`が、各ノートを `notes/<noteId>.md` として書き出す
(YAML front matter: id/title/tags/created_at/updated_at、AI要約は source_note_id/generated_by)。
タグ検索の正本形式(ユーザー設計)。App.tsxがnotes変更のたびdebounceし、**変更のあった非空・
非junkノートだけ**書く(全501件を毎回書かない。sigをrefで比較)。front matterのYAMLスカラーは
yamlScalar()で最小限クォートし、読み側(native-host/build_index.py)と規則を合わせる。
索引(SQLite)はnative-host側で.mdから再生成する——正本はあくまで.md。
削除ノートの.mdはNAS上に残る(ネイティブホストにdelete未実装。既知の割り切り)。

## active/New Tab Board.txt は「出先で確認」用の単一ミラー(履歴とは別物)

`writeActiveNotesToNas`は、日付フォルダの履歴とは別に、現在のボード上の全ノートを
`active/New Tab Board.txt`という**固定名の単一ファイル**へまとめて書く(ユーザー指示: 出先で
1ファイル開けば全ノートを読める)。各ノートは`title: <ノート名>\n\n<本文>`で連結する。
App.tsxがnotes変更のたびdebounce(3秒)して自動ミラーする。**このファイルはアプリから読み戻さない**
(履歴の復元/diff経路ではない)ので、getSnapshotBodyのような正規化は不要——書き専用。
