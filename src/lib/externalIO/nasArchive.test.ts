...
describe("flushSnapshotToNas", () => {
  it("contentが無ければfalse(二重フラッシュ防御)", async () => {
    ...
    expect(await flushSnapshotToNas(NAS_PATH, { ...baseSnapshot, content: undefined }, nas)).toBe(
      false,
    );
  });
  ...
  it("書き込み+再読込一致でtrue", async () => {
    const nas = makeFakeNas();
    expect(await flushSnapshotToNas(NAS_PATH, baseSnapshot, nas)).toBe(true);
  });

  it("書き込み自体が失敗すればfalse", async () => {
    const nas = makeFakeNas({ writeShouldFail: true });
    expect(await flushSnapshotToNas(NAS_PATH, baseSnapshot, nas)).toBe(false);
  });
...
describe("flushAllToNas(パス・NASクライアントを依存注入)", () => {
  ...
  it("readArchivedSnapshotは存在しないファイルパスならnull", async () => {
    const nas = makeFakeNas();
    expect(
      await readArchivedSnapshot("missing.snapshot", {
