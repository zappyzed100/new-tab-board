...
describe("syncNoteToDrive", () => {
  ...
  it("driveFileId未設定なら検索してから新規アップロードする", async () => {
    ...
    expect(findFileForNote).toHaveBeenCalledWith("n1", "token-abc");
    expect(uploadNote).toHaveBeenCalledWith(note, "token-abc", null);
  });
  ...
  it("driveFileId既知なら検索をスキップして更新アップロードする", async () => {
    ...
    expect(uploadNote).toHaveBeenCalledWith(withFileId, "token-abc", "existing-file");
  });
