...
- [x] 初期化/履歴復元/取込/固定タグ追記で古いCM6の遅延イベントがnote.contentを上書きするCI専用レースを断つ
- [x] お気に入り一覧の凍結項目をダブルクリックで盤面へ回収する（保管庫からメモを取り戻す導線）

### 意図的な本文置換で、古いCM6の遅延イベントがnote.contentを上書きするCI専用レース(2026-07-27)

2026-07-24の修正(018898f)は「再マウントがドラフトと note.content のどちらを読むか」だけを
直しており、**note.content 自体**が意図的置換の直後に古いCM6インスタンスからの遅延
onContentChangeで上書きされる経路は塞げていなかった。CI(Linux/xvfb)で再発:
初期化ボタンを押した直後の本文が空でなく、末尾が数文字欠けた値(打っていた本文の途中)に
なっていた。ローカル(Windows・高速)では再現しない——CJKの疑似合成/CM6の内部処理が
実機より遅れて発火することがある、という2026-07-24時点の仮説と符合する。

対策は「原因(なぜ遅れるか)を追わず、結果を止める」: 意図的置換の4箇所(初期化/履歴復元/
ファイル取込/固定タグのblur追記——いずれも clearDraft→再マウントの形)で
`suppressContentChangeRef` を同期的に立て、`onContentChange` はこのフラグが立っている間
一切の書き込みを無視する。フラグは新インスタンスの再マウントが完了するeffect
(`[restoreCounter, replaceContentVersion]`)で下ろす——それより前に古いビューが発火する
イベントは、原因を問わず「もう関係ない」と扱ってよい。

このCI専用レースは、018898f時点でも「ローカル再現不可・CI Linux/xvfbでのみ決定的に赤」
だった前歴があり、今回も同じ理由でローカルでは再現できていない(このマシンはWindows)。
既存の関連E2E 27本(初期化/履歴復元相当/取込/固定タグ追記)はすべてローカルで緑のまま。
...
ユーザー報告のエラーそのまま: `interactive:true`(明示的なボタンクリック)で呼んでいるのに、
非対話フロー向けのエラー(`abortOnLoadForNonInteractive`等への言及)が出る。ブラウザプロフィールが
既にサインイン済み+同意済みだと、Googleの認可ページが実際の画面遷移を1回も発生させずに
即座にリダイレクトで完了することがあり、`launchWebAuthFlow`は`interactive:true`でも
「ユーザーが実際に何か操作したか」を見ているらしく、無操作で完結すると非対話時と同じ
エラーを投げる(Chrome側の実装。googleAuth.tsのヘッダー参照)。

対話フローの認可URLにだけ`prompt=select_account`を足し、アカウント選択という実際のクリックを
1回強制することで回避する。非対話(バックグラウンド定期同期)側には付けない——付けると
2026-07-20に潰した「1時間ごとの再認可」が別の形で復活する。**実機のOAuthフローを伴うため
このサンドボックスでは検証できていない**——ユーザーの実機での再テストが必要。

### UI表示文言の改称: 「NAS」→「保管庫」・「スペシャル」→「お気に入り」(2026-07-27)

ユーザー指摘「NASって名前にしてたけど不適格」「スペシャルもお気に入りだな」。**画面に出る文言と
ドキュメントのみ**を改称した——コード側の識別子・関数名・ファイル名・保存キー・front matter・
ネイティブホスト名・画像スキーム(`nas:`)は据え置き(データ互換とネイティブホスト再登録の回避)。

### お気に入りの凍結項目をダブルクリックで盤面へ回収する(2026-08-04)

ユーザー指示「お気に入りリストからリストをダブルクリックしたら、保管庫からメモを回収して
表示し直すようにしてほしい」。⭐のノートを削除すると凍結項目(`SpecialItem`)として一覧に残るが、
これまで一覧からできるのは「お気に入りから外す(=凍結項目の削除)」だけで、**中身を読み返す/
盤面へ戻す導線が無かった**——保管庫(NAS)の `special/<folder>/<id>.md` を直接開くしかなかった。

設計判断は3つ:

- **本文は凍結時のスナップショットから戻す**(保管庫のファイルを読み直さない)。`SpecialItem` は
  本文をそのまま持っており、これは保管庫/Driveの special ミラーへ書き出しているのと同じ中身
  (`specialSync.ts`/`driveSpecial.ts`)。凍結後は誰も編集しないので両者は一致し、ローカルから
  戻せば保管庫が未設定/未接続の端末でも回収できる(ネイティブホストへの往復も要らない)。
- **idは引き継ぎ、`updatedAt` は回収時刻で上書きする**。idを引き継ぐのは履歴スナップショット
  (IndexedDBの`noteId`)と保管庫/Driveのファイル名が同じidで紐づいているため——新idで戻すと
  回収したメモが自分の過去と切り離される。`updatedAt` の上書きは必須で、削除時に打たれた
  tombstone より新しくないと `commitNoteMutation` の upsert が「削除済みの古いノート」と
  見なして復活そのものを捨てる(逆に新しければ tombstone 側が外れ、削除の取り消しが全タブへ
  収束する)。E2Eの再読込アサーションがこの不等号を固定している。
- **回収と同時に凍結項目は外す**。`specialEntries` は同idならliveを優先するため、凍結項目を
  残しても一覧の見た目は変わらないが、同じメモの古いコピーが次の削除まで誰も更新しないまま
  残り続ける。回収してもノートの `special` は true のままなので、一覧上は同じ行が
  「凍結」から通常のノートへ変わるだけに見える。

単クリックではなくダブルクリックなのはユーザー指定どおり(盤面を書き換える操作を、一覧を
眺めるだけのクリックで暴発させない)。凍結項目は `<Text>` から live と同じ ghost `<Button>` へ
変え、「凍結」バッジはボタンの外へ出した——ボタンの中に入れると行の高さがlive行より4px高く
なり、ボタン化前は凍結タイトルの左端がlive行から8pxずれていた(どちらも実測で検出。E2Eが数値で固定)。
...
### 履歴スナップショットは「マウント」を編集と見なさない(2026-07-25の重さの原因)
...
編集していなくても**ペインがマウントしただけで1件保存**されていた。ペインは窓化
(`ViewportNote`)によりスクロールのたび・タブを開くたびにマウントするので、無編集のまま
際限なく増える。実測(300ノート): 初回ロード18件 → 上下スクロール1往復ごとに+20件 →
タブを3回開閉するだけで+48件(合計126件・内容はすべて同一)。

**なぜ「だんだん」重くなるか**: 1件ごとに gzip + IndexedDB put に加えて `indexSnapshot` が
走り、これは本文の全トークンについて転置索引の `refs` 配列を読んで `[...refs, id]` で
書き戻す。`refs` は溜まったスナップショット数に比例して伸びるため、**溜まるほど1件の
保存が重くなる**(`refs.includes` の線形走査も同じ)。使うほど遅くなり、最後に固まる。

対策は2段。①`lastContentRef` の初期値をマウント時点の本文にする(マウントは変更ではない)。
②保存済みの最新スナップショットと `contentHash` が同じなら書かない——①はタブごとの
メモリ上の前回内容しか見ないため、複数タブが同じ編集を受け取ると同一内容がタブの数だけ
積まれる。明示保存(`forceSnapshot`)は「呼ぶたびに1件残す」契約なので②の対象外。
回帰は `e2e/stress/history-growth.spec.ts` が「スクロール・タブ開閉で1件も増えない/
編集すれば増える」を数える(修正前は初回ロードの時点で18件になり赤)。

**既に溜まった分の後始末**は `history/snapshotCleanup.ts`(データ管理の「履歴の重複を掃除」)。
同じノートの中で**連続する**同一内容を最初の1件へ畳み、転置索引の `refs` からも参照を落とす
——索引を放置すると refs が伸びたままで、重さの本体(1件あたりの書き戻し量)が減らない。
比較キーは `contentHash`(新しい実装が書く)→ 無ければ圧縮済み本文(gzipは同じ入力なら同じ
出力なので古い分も比較できる)。**archived(本体がNAS)と比較不能なものは常に残す**——
消す側でなく残す側へ倒す。A→B→A は連続していないので両方のAを残す。履歴を消す操作なので
UIは二段クリック(押す→赤い確認ボタン)にした(`window.confirm` はこのアプリで未使用)。

### 固定タグモードは「blurで本文へ書き、盤面はフィルタするだけ」(2026-07-25)

ユーザー指示「最初に固定タグ(複数可)を作り、編集するノートに全てのタグを追加、表示するノートも
そのタグ条件を全て満たすものだけにする」「固定タグを登録して選択式で切り替えられるように」。
プリセット(`Settings.fixedTagPresets` + `activeFixedTagPresetId`)を1つ選ぶとモードONになる
——ON/OFFの別フラグは持たない(フラグとidが食い違う状態を作らないため。`activeFixedTags` が
空配列を返すことが唯一のOFF判定)。
...
同じ再マウント経路(`clearDraft` + `replaceFromNoteContent` + `restoreCounter`)が要り、それを
入力中に走らせるとカーソルが飛ぶ。blurなら編集していないので実害が無い。**空ノートには付けない**
(ユーザー指示)——末尾の空プレースホルダが汚れると `isGeneratedEmptyPlaceholder` から外れて
「常に空3つ」の補充が暴れる。

**登録は全タブ共有・選択はタブ毎**(ユーザー指示「タブ毎に切り替えられるように。ノート毎じゃない」)。
プリセットの登録内容だけが `Settings.fixedTagPresets`(chrome.storage.local=全タブ共有・永続)で、
**どれを選んでいるかは `sessionStorage`**(`storage/tab-session.ts`)に置く——sessionStorage は
タブごとに独立し、そのタブのリロードでは残り、タブを閉じれば消える。選択を設定側へ入れると
あるタブでの切替が全タブへ伝播し、「タブAは仕事・タブBは勉強」ができない。新しいタブは
常に「(なし)」=全件表示から始まる。

**盤面の絞り込みは `visibleNotes` 一箇所**(App.tsx)。素通しするのは「空プレースホルダ」と
「編集中」の2種だけ——前者が無いとモード中に書き始める場所が無くなり、後者が無いと空ノートへ
1文字目を打った瞬間に(まだタグが付いていないので)自分が盤面から消える。**選択中(activeNoteId)は
素通ししない**: 条件を満たさないノートが1件だけ残り続け、絞り込みの意味が壊れるため。
隠すだけでデータは消さない——`notes` 自体は全件のまま保存・同期される。
...
**保存先は `images/<noteId>/<YYYY-MM-DD>-<uuid>.<ext>`**(2026-07-23)。フォルダをノートidで切るのは、
「このノートの画像=このフォルダ」という規則がサムネイル帯の土台だから(本文の参照だけを見ると、
参照テキストを消した瞬間に画像を見失う)。日付フォルダ階層(`images/2026/7/23/…`)は採らなかった
——ノートの日次アーカイブ `YYYY/M/D/<id>.md` は「その日の盤面を残す」ためのものだが、添付画像は
生きているノートから参照され続けるので性質が `active/` 側であり、日付で切るとパスから
「どのノートのものか」が消えてファイル名へidを埋め直す必要が出る。一方で「いつ貼ったか」は
NAS上で辿れた方がよいので、**ファイル名の先頭に貼り付け日**を置いた(ユーザー判断)。日付は
**ゼロ埋めする**——日付フォルダ(ユーザー指示でゼロ埋めしない)と違い、ここは同じフォルダ内で
名前順に並べたときに時系列になることが目的だから。既存ファイルは移行不要(照合はフォルダ単位)。
