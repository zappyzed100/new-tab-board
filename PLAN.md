...
- 予定前アラーム: Googleカレンダーの次の予定を15分毎にポーリングし、開始10分前に1回だけ鳴らす
  （同じ予定に対して再発火しない）。
- バッテリー低下警告: GAS Web Appを15分毎にポーリングし、10/20/50%の閾値を新たに下回るたびに
  1回ずつ鳴らす（回復でリセット）。予定前アラームとオフスクリーンのループ音声を共用する。
...
### スマホのバッテリー低下警告(GAS Web App中継・2026-07-16)
...
オフスクリーンのループ音声)を再利用し、10/20/50%の閾値を新たに下回った時だけ発火する
(`src/lib/battery/batteryAlarm.ts`の`decideBatteryAlarm`。回復でリセット)。予定前アラームと
オフスクリーン音声を共用するため、どちらか片方の「停止」でもう片方が鳴動中なら音声は止めない
(`fireAlarm`/`stopAlarm`/`fireBatteryAlarm`/`stopBatteryAlarm`のhasDocument()相互確認)。
新設 `src/lib/battery/`(閾値判定の純関数)・`gas/`(GASスクリプト本体。npm/vitest/pytestの
対象外・script.google.comへ手動デプロイ)。接続設定(URL+共有トークン)はDataPanelから
Gemini APIキーと同じ「秘匿情報として画面に出さない」パターンで保存。
