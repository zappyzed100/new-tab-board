...
| データ管理(Drive自動バックアップ/ファイル/NAS設定) | DataPanel | `src/newtab/components/shell/DataPanel.tsx` | `data-panel` | 常時表示・パネル(トグルボタンは撤去済み) |
...
## 補足
- 「常時表示」でも実際にはトグルボタン側(`toggle-search`等)がApp.tsx側にあり、その操作対象として上記パネル部品が出てくる形のものが多い(SearchPanel/HistoryPanel)。DataPanelは2026-07にトグルを撤去し常時表示化。
