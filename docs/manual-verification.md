...
- [ ] 「フォルダへ書き出し」でネイティブのフォルダ選択ダイアログが1回出て、選んだ
      フォルダへ全ノートが`.md`ファイルとして書き出されるか(**既知の問題**:
      「NASフォルダを設定」と同じくshowDirectoryPickerを使うため、Chromium拡張機能
      コンテキストの既知バグ(WICG/file-system-access#314、crbug.com/issues/40240444)
      の影響を受ける。フォルダを選んでも「キャンセルされたか、選択後に失敗しました」
      と出た場合、Chromeを最新版に更新のうえ再試行する)
