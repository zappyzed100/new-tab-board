...
- [ ] 「フォルダへ書き出し」でノートの件数ぶん、ネイティブの保存先選択ダイアログが
      1件ずつ順番に出て、選んだ場所へ`.md`ファイルとして書き出されるか(以前は
      showDirectoryPickerの既知バグにより拡張機能コンテキストで無反応になっていた
      ため、chrome.downloads の saveAs へ変更した——1件ごとに保存先を選ぶ設計になった
      点に注意。件の途中でダイアログをキャンセルすると、そこまでの件数を表示して
      打ち切られるか)
