<!-- PLAN_FORMAT.md — このリポジトリのPLAN.mdが従うべき書式の見本(別プロジェクト
     「Progress Proof」のPLAN.mdをそのまま複製したもの・2026-07-16 ユーザーが雛形として配置)。
     内容(目的/アーキテクチャ等)はProgress Proof固有で本プロジェクトとは無関係——
     書式(節構成・タスクのチェックボックス記法)だけを本プロジェクトのPLAN.mdへ流用する。
     本プロジェクトのPLAN.mdを編集する時はこの書式を踏襲すること(AGENTS.md §4参照)。 -->
# PLAN.md — Progress Proof 全体計画（書式サンプル。内容は他プロジェクトのもの）
