...
- **v2.25（G7）— compact 再発火バグの修正**: SessionStart は「1セッション1回のみの
  発火」という v2.23 時点の前提が誤りだった。実際には `source`
  （`"startup" | "resume" | "clear" | "compact"`）が入力に含まれ、**compact（要約）
  でも同じ `session_id` のまま再発火する**（実測: transcript 内で `isCompactSummary`
  の前後とも `sessionId` が不変）。この再発火時、その瞬間 dirty だった AI 自身の
  継続作業（要約直前の未コミット編集）が「人間の WIP」として baseline に**上書き**
  され、以後セッションの残り全体でそれらのファイルへの Edit/Write が誤ってブロック
  され続ける実害が確認された。対処: `session_baseline.py` は `source == "compact"`
  の時、`git status` を呼ばず baseline ファイルにも一切触れずに `return`
  する（空文字列で上書きするのは不可——それだと真の startup 時点の正当な baseline
  内容を破壊し、今回発生したのと同じ事故になる）。`source` が取得できない/想定外の
  値の場合は安全側に倒し、従来通り baseline を書く。回帰は
  `tests/test_session_baseline.py`（`uv run python tests/test_session_baseline.py`）
  が担保する。
...
- **検査2（fix ⇔ 回帰テストの対・`fix-without-test`）**: メッセージが `fix:` で始まるとき、
  ...
  **v2.25（G10）是正**: 検査6・8（soft）は`TEST_PATH_PATTERNS`未充填なら不発という
  バイパスを持つが、検査2（hard）だけこのバイパスが無い非対称バグがあった——列充填前の
  リポジトリ（採用列の binding 未実施・またはキット原本自身）では `fix:` コミットが
  原理上常に拒否される実害があった。検査2にも同じバイパスを追加して揃えた
  （回帰は `tests/test_check_commit_msg.py`）。
...
- ✅ **`commit-msg-history` ジョブ（PR のみ — v2.25・G10 是正）**: `pre-commit run
  --all-files` は `stages: [pre-push]` だけでなく `stages: [commit-msg]`
  （`check-commit-msg`）も対象外——`pre-commit install` していない環境（GitHub の
  Web UI 経由のコミット等）からは commit-msg 段の HARD 検査群（fix⇔テスト・
  governance-without-goal・undeclared-dependency・feat-without-plan）が一切効かない
  実害があった。`scripts/check_commit_msg_history.py` が PR 範囲（base..head・
  マージ除外）の各コミットを一時 worktree へ `git worktree add` + `git reset --soft`
  で再現し、`check_commit_msg.py` を無改造のまま1コミットずつ再生する
  （`red-first` ジョブと同じ「一時 worktree + 差し替え呼び出し」流儀）。
...
| 32 | new-tab-board 導入時の未実測項目の登録（2026-07-08・Step 10 総合監査） | §2・§5 | 🚧 |
...
### Phase 32 — new-tab-board 導入時の未実測項目（2026-07-08・Step 10 総合監査）🚧

本リポジトリのブートストラップ（Step 0〜9）で、実装済み・列充填済みだが違反注入で
未実測のまま残った規則・機構を記録する（実行規律5「省略記法での完了扱いの禁止」の
対象外——「実測できなかった理由」を明示した上での意図的な保留）。

- 🚧 **`guard-corpus-mismatch`（規則1つを無効化して赤化を確認する実測）**: 実測しようと
  `guard_git_bypass.py` の1ルールを一時的にコメントアウトしたところ、本セッションの
  実行ハーネス自身の安全分類器に「ガードレールの自己弱体化」としてブロックされた
  （ユーザー承認を得た上での再試行も同様にブロック）。ロジック自体は全74行のコーパス
  再生 PASS で健全性を確認済みだが、「壊した状態で赤くなることを直接見る」形の実測は
  本環境からは実施不能。トリガー: 別の信頼済み CLI セッションから再試行する機会があれば。
- 🚧 **`hooks-path-overridden`（`.git/config` へ `core.hooksPath` を直接書き込んで検出を
  見る実測）**: `git config core.hooksPath` 経由は `guard_git_bypass.py` 自身が正しく
  ブロックすることを確認済みだが、`.git/config` を直接編集してこの検査を試そうとした
  ところ、実行ハーネスの安全分類器に「ツールを変えての deny 回避」としてブロックされた
  （正当な制限と判断し撤退）。トリガー: 同上。
- 🚧 **`RED-FIRST-EXEMPT:` の免除経路そのもの**: `red-first-green` が親で緑のテストを
  正しく検出することは検証用PRで実測済み（§5）。免除コメント自体の効果（理由ありで
  免除される・理由なしなら不成立のまま）は未実測。トリガー: 次に本当に CI 上で
  再現不能な fix を書く時に自然と実測される。
- — **`missing-catch-unwind` / `test-calls-solver-direct`**: 対象外（表B: FFI境界も
  確率的コンポーネントも本プロジェクトには存在しない）。
