...
```json
{ "type": "write-file", "path": "Z:\\NAS\\backup", "filename": "n1-123-s1.snapshot", "content": "本文" }
```
...
```json
{ "type": "read-file", "path": "Z:\\NAS\\backup", "filename": "n1-123-s1.snapshot" }
```
...
await writeFileToNas("Z:\\NAS\\backup", "foo.snapshot", "本文"); // 成功/失敗をboolean で返す
const content = await readFileFromNas("Z:\\NAS\\backup", "foo.snapshot"); // 読めなければnull
```
