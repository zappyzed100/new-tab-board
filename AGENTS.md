...
- 設計根拠 = `PLAN.md`（無ければ作成時に。2026-07-16に`plan.md`からリネーム——外部ツール
  「Progress Proof」が複数リポジトリ横断でタスクを収集する雛形フォーマットに合わせた）。
  **書式は `docs/PLAN_FORMAT.md`（他プロジェクトのPLAN.mdをそのまま複製した見本。中身は
  無関係・節構成とタスク記法だけを踏襲する）を厳守する**: 目的・アーキテクチャ・
  技術選定理由・運用・ロードマップに加え、タスクは機械可読なチェックボックス記法
  （`- [ ] タイトル` / `` `next`/`in_progress`/`blocked` ``等のタグ）で書く。
  PLAN.mdを編集する時は必ず`docs/PLAN_FORMAT.md`を読んでから節構成を合わせること。
  **レイヤー直下に新規ディレクトリを作る
...
- `chrome.storage.local` はクォータ 10MB（Manifest V3・無制限化オプション未使用時）。
  ...
  元々ブックマーク等の軽量データをsyncに乗せていたが、1アイテムにまとめていたため8KB上限
  超過で書き込みが無音で失敗しブックマークが消える実害が出たため、**現状は`local`のみを
  意図的に使用**している(`src/lib/storage/storage.ts`のシームに集約。2026-07-18)。複数端末間の
  自動同期はsyncの役目を諦め、Drive/NAS JSONバックアップの手動復元に委ねる。
